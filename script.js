...
function sortPokemons(argument){
    let validInput = ["type", "base_damage", "base_hp", "sped"]
...
function filterPokemons(argument){
    ...
    let result = filteredPokemons.length === 0 ? "Tipo de pokemom no encontrado" : filteredPokemons
...
let pokemonMaster = 
    {id:1, number:2, name: "drico", created_date: "22 de noviembre", pokemon: []};
...
function addRandomPokemon(){
    const aleatorio = Math.round(Math.random()*pokemons.length)
